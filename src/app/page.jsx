    ...
    <div className={`group relative rounded-[30px] md:rounded-[40px] overflow-hidden ${styling.bg} ${styling.shadow} hover:scale-[1.02] transition-all duration-500`}>
      <div className="relative h-56 md:h-72">
        ...
        <div className="absolute top-4 left-4">
          <Badge className={`${styling.badge} font-semibold text-xs px-3 py-1`}>
            {salon.gender}
          </Badge>
        </div>
      </div>
      ...
      <div className="p-6 md:p-8">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h3 className="text-xl md:text-2xl font-bold text-white mb-2">{salon.name}</h3>
            <div className="flex items-center gap-2 text-white/80 text-sm mb-3">
              <MapPin className="h-4 w-4" />
              <span>{salon.area || salon.location}</span>
            </div>
            <div className="flex items-center gap-1 mb-4">
              {[...Array(5)].map((_, i) => (
                <Star 
                  key={i} 
                  className={`h-4 w-4 ${i < 4 ? 'fill-yellow-400 text-yellow-400' : 'text-white/30'}`} 
                />
              ))}
              <span className="text-white/80 text-sm ml-2">4.5 (128)</span>
            </div>
          </div>
        </div>
        ...
        <Link 
          ...
          className={`block w-full ${styling.button} text-white font-semibold py-3 px-6 rounded-xl text-center transition-all duration-300 hover:shadow-lg`}
        >
          Book Now
        </Link>
      </div>
...
export default function Home() {
  ...
  const fetchSalons = async () => {
    try {
      ...
      if (result.success) {
        setSalons(result.data.all || []); // Show all salons
      }
    } catch (error) {
  ...
  // Memoize filtered salons to prevent unnecessary recalculations
  const filteredSalons = useMemo(() => {
    const maleSalons = salons.filter(salon => salon.gender === 'male');
    const femaleSalons = salons.filter(salon => salon.gender === 'female');
    const unisexSalons = salons.filter(salon => salon.gender === 'unisex');
    
    return { maleSalons, femaleSalons, unisexSalons };
  }, [salons]);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {[...Array(6)].map((_, i) => (
        <div key={i} className="animate-pulse">
          <div className="bg-gray-300 rounded-[30px] h-72 mb-4"></div>
  ...
  // Get styling based on salon gender
  const getSalonCardStyling = (gender) => {
    switch (gender) {
      case 'male':
        return {
          bg: 'bg-[#1e3a8a]',
  ...
  const SalonCard = ({ salon }) => {
    const styling = getSalonCardStyling(salon.gender);
  ...
  const heroImage = PlaceHolderImages.find(img => img.id === 'salon-hero')?.imageUrl || "https://picsum.photos/seed/salon-hero/1200/800.jpg";
  ...
  const categories = [
    ...
    { 
      ...
      btn: "bg-pink-600 hover:bg-pink-700 shadow-[0_0_20px_rgba\\(219,39,119,0.4\\)]",
      link: "/salons/female"
    },
    { 
      id: 'unisex',
      ...
      btn: "bg-purple-600 hover:bg-purple-700 shadow-[0_0_20px_rgba\\(147,51,234,0.4\\)]",
      link: "/salons/unisex"
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-background selection:bg-primary/30 font-body scroll-smooth">
      ...
      <main>
        ...
        <section className="relative h-[80vh] md:h-[90vh] flex items-center justify-center overflow-hidden">
          ...
          <div className="relative z-10 text-center space-y-6 md:space-y-8 px-6 max-w-4xl mx-auto">
            ...
            <div className="pt-4 md:pt-8">
              <Button asChild className="rounded-full px-8 md:px-12 h-14 md:h-16 text-lg md:text-xl bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white border-none shadow-[0_0_20px_rgba\\(168,85,247,0.5\\)] transition-all duration-300">
        ...
        {/* 2. Tailored For You - Three Category Cards */}
        <section id="tailored-salons" className="container mx-auto px-4 py-12 md:py-20 space-y-8 md:space-y-12 scroll-mt-24">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-10">
            {/* Male Salons Card */}
            <div className="group relative h-[350px] md:h-[600px] rounded-[30px] md:rounded-[40px] overflow-hidden border-2 transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl border-blue-500 shadow-blue-500/30">
              <Image 
                src={PlaceHolderImages.find(img => img.id === 'category-male')?.imageUrl || "https://picsum.photos/seed/male/600/800"}
                alt="Male Salons" 
                fill 
                className="object-cover transition-transform duration-[2000ms] group-hover:scale-110" 
                data-ai-hint="barbershop luxury"
              />
              <div className="absolute inset-0 transition-opacity duration-500 group-hover:opacity-60 bg-blue-900/40">
                <div className="absolute inset-x-4 bottom-4 md:inset-x-6 md:bottom-6 p-6 md:p-8 rounded-[24px] md:rounded-[30px] bg-white/10 backdrop-blur-xl border-white/20 shadow-2xl flex flex-col gap-3 md:gap-4 transform transition-all duration-500 group-hover:-translate-y-2">
                  <h3 className="font-headline text-2xl md:text-4xl text-white drop-shadow-md">Male Salons</h3>
                  <p className="text-white/80 text-sm md:text-lg leading-snug drop-shadow-sm">Premium barbershops & grooming services for men</p>
                  <Button asChild className="w-full rounded-full h-10 md:h-12 text-sm md:text-base text-white font-semibold transition-all duration-300 border-none mt-2 bg-blue-600 hover:bg-blue-700 shadow-[0_0_15px_rgba(37,99,235,0.4)]">
            ...
            {/* Female Salons Card */}
            <div className="group relative h-[350px] md:h-[600px] rounded-[30px] md:rounded-[40px] overflow-hidden border-2 transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl border-pink-500 shadow-pink-500/30">
              <Image 
                src={PlaceHolderImages.find(img => img.id === 'category-female')?.imageUrl || "https://picsum.photos/seed/female/600/800"}
                alt="Female Salons" 
                fill 
                className="object-cover transition-transform duration-[2000ms] group-hover:scale-110" 
                data-ai-hint="spa beauty"
              />
              <div className="absolute inset-0 transition-opacity duration-500 group-hover:opacity-60 bg-pink-900/40">
                <div className="absolute inset-x-4 bottom-4 md:inset-x-6 md:bottom-6 p-6 md:p-8 rounded-[24px] md:rounded-[30px] bg-white/10 backdrop-blur-xl border-white/20 shadow-2xl flex flex-col gap-3 md:gap-4 transform transition-all duration-500 group-hover:-translate-y-2">
                  <h3 className="font-headline text-2xl md:text-4xl text-white drop-shadow-md">Female Salons</h3>
                  <p className="text-white/80 text-sm md:text-lg leading-snug drop-shadow-sm">Beauty parlors & spa services for women</p>
                  <Button asChild className="w-full rounded-full h-10 md:h-12 text-sm md:text-base text-white font-semibold transition-all duration-300 border-none mt-2 bg-pink-600 hover:bg-pink-700 shadow-[0_0_15px_rgba(236,72,153,0.4)]">
                    <Link href="/salons/female">Explore Salons</Link>
                  </Button>
                </div>
              </div>
            </div>
            ...
            {/* Unisex Salons Card */}
            <div className="group relative h-[350px] md:h-[600px] rounded-[30px] md:rounded-[40px] overflow-hidden border-2 transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl border-purple-500 shadow-purple-500/30">
              <Image 
                src={PlaceHolderImages.find(img => img.id === 'category-unisex')?.imageUrl || "https://picsum.photos/seed/unisex/600/800"}
                alt="Unisex Salons" 
                fill 
                className="object-cover transition-transform duration-[2000ms] group-hover:scale-110" 
                data-ai-hint="unisex salon"
              />
              <div className="absolute inset-0 transition-opacity duration-500 group-hover:opacity-60 bg-purple-900/40">
                <div className="absolute inset-x-4 bottom-4 md:inset-x-6 md:bottom-6 p-6 md:p-8 rounded-[24px] md:rounded-[30px] bg-white/10 backdrop-blur-xl border-white/20 shadow-2xl flex flex-col gap-3 md:gap-4 transform transition-all duration-500 group-hover:-translate-y-2">
                  <h3 className="font-headline text-2xl md:text-4xl text-white drop-shadow-md">Unisex Salons</h3>
                  <p className="text-white/80 text-sm md:text-lg leading-snug drop-shadow-sm">Complete grooming services for all genders</p>
                  <Button asChild className="w-full rounded-full h-10 md:h-12 text-sm md:text-base text-white font-semibold transition-all duration-300 border-none mt-2 bg-purple-600 hover:bg-purple-700 shadow-[0_0_15px_rgba(147,51,234,0.4)]">
                    <Link href="/salons/unisex">Explore Salons</Link>
                  </Button>
                </div>
              </div>
            </div>
            
            {/* Duplicate Female and Unisex cards removed */}
          </div>
        </section>
            ...
            <div className="grid lg:grid-cols-2">
              ...
              <div className="relative h-[400px] md:h-[600px] overflow-hidden group">
                <div className="absolute top-4 left-4 md:top-6 md:left-6 z-20 flex items-center gap-2 bg-slate-900/90 backdrop-blur-md px-3 py-1.5 md:px-4 md:py-2 rounded-full border border-white/10 shadow-xl">
                  ...
                  <span className="text-[10px] md:text-xs font-semibold text-white tracking-wide">LIVE MAP</span>
                </div>
                ...
                {/* Salon Markers Overlay */}
                <div className="absolute inset-0 z-15 pointer-events-none">
                  {/* Female Salon Markers */}
                  {filteredSalons.femaleSalons.slice(0, 3).map((salon, index) => (
                    <div
                      key={salon._id}
                    ...
                    >
                      <div className="relative">
                        <div className="h-3 w-3 md:h-4 md:w-4 bg-pink-500 rounded-full shadow-[0_0_15px_rgba\\(236,72,153,0.8\\)] animate-pulse" />
                        <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-slate-900/95 backdrop-blur-md px-2 py-1 rounded-lg border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                          <p className="text-[10px] text-white font-semibold">{salon.salonName || salon.name}</p>
                          <p className="text-[8px] text-pink-400">★ {salon.rating || '4.5'}</p>
                        </div>
                      </div>
                    </div>
                  ))}
                  ...
                  {/* Male Salon Markers */}
                  {filteredSalons.maleSalons.slice(0, 2).map((salon, index) => (
                    <div
                      key={salon._id}
                      className="absolute pointer-events-auto cursor-pointer group"
                      style={{
                    ...
                    >
                      <div className="relative">
                        <div className="h-3 w-3 md:h-4 md:w-4 bg-blue-500 rounded-full shadow-[0_0_15px_rgba\\(59,130,246,0.8\\)] animate-pulse" />
                        <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-slate-900/95 backdrop-blur-md px-2 py-1 rounded-lg border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                          <p className="text-[10px] text-white font-semibold">{salon.salonName || salon.name}</p>
                          <p className="text-[8px] text-blue-400">★ {salon.rating || '4.5'}</p>
                        </div>
                      </div>
                    </div>
                  ))}
                  
                  {/* Unisex Salon Markers */}
                  {filteredSalons.unisexSalons.slice(0, 2).map((salon, index) => (
                    <div
                      key={salon._id}
                      className="absolute pointer-events-auto cursor-pointer group"
                      style={{
                    ...
                    >
                      <div className="relative">
                        <div className="h-3 w-3 md:h-4 md:w-4 bg-purple-500 rounded-full shadow-[0_0_15px_rgba\\(168,85,247,0.8\\)] animate-pulse" />
                        <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-slate-900/95 backdrop-blur-md px-2 py-1 rounded-lg border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                          <p className="text-[10px] text-white font-semibold">{salon.salonName || salon.name}</p>
                          <p className="text-[8px] text-purple-400">★ {salon.rating || '4.5'}</p>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="p-8 md:p-16 space-y-8 md:space-y-10 flex flex-col justify-center bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 text-white">
                ...
                <div className="pt-6 md:pt-8 grid grid-cols-3 gap-2 md:gap-4 border-t border-white/5">
                  <div className="flex items-center gap-2">
                    <div className="h-2 w-2 md:h-3 md:w-3 rounded-full bg-pink-500 shadow-[0_0_25px_rgba\\(236,72,153,0.9\\)]" />
